...
TTIMED=0
INDEX=1
OUT_TIMEDOUT=required_poolswithstates_timedout.ini
cat /dev/null>${OUT_TIMEDOUT}
for line in `cat vmpoolswithstates_${OS}.txt|cut -f1 -d':'`
do
  POOL="$line"
  #echo ansible ${POOL} -i cbqe_vms_per_poolswithstates_${OS}.ini -u root -m ping
  #ansible ${POOL} -i cbqe_vms_per_poolswithstates_${OS}.ini -u root -m ping |tee ping_log_${OS}_${POOL}.txt
